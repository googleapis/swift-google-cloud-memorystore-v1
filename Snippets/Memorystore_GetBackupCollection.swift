...
func sample(
  ...
  let response = try await client.getBackupCollection(
    request: GetBackupCollectionRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/backupCollections/\(backupCollectionId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        backupCollectionId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
